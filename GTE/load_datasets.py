...
class Datasets(Dataset):
    def __init__(self, args, split='train'):
        ...
        if self.task_name == 'SNLI':
            self.data = np.load('/data/zhangdacao/dataset/snli/'+split+'.npy', allow_pickle='TRUE')
        elif self.task_name == 'SNLI-VE':
            self.image = np.load('/data/zhangdacao/dataset/snli_ve/image.npy', allow_pickle='TRUE').item()
            self.data = np.load('/data/zhangdacao/dataset/snli_ve/'+split+'.npy', allow_pickle='TRUE')
        elif self.task_name == 'GTE':
            self.image = np.load('/data/zhangdacao/dataset/GTE/clip_image.npy', allow_pickle='TRUE').item()
            self.data = np.load('/data/zhangdacao/dataset/GTE/'+split+'.npy', allow_pickle='TRUE')
        elif self.task_name == 'MultiNLI':
            if split == 'train':
                self.data = np.load('/data/zhangdacao/dataset/multi_nli/train.npy', allow_pickle='TRUE')
            elif split == 'validation':
                self.data = np.load('/data/zhangdacao/dataset/multi_nli/validation_matched.npy', allow_pickle='TRUE')
            else:
                self.data = np.load('/data/zhangdacao/dataset/multi_nli/validation_mismatched.npy', allow_pickle='TRUE')
    ...
    def __getitem__(self, idx):
        if self.task_name == 'GTE':
            ...
            return {'image':image, 
                    ...
                    'label':label}
        elif self.args.task_name != 'SNLI-VE':
        ...
        if self.task_name == 'SNLI' or self.task_name == 'MultiNLI':
            text1, text2, label = self.data[idx]['premise'], self.data[idx]['hypothesis'], self.data[idx]['label']
...
class MultiNLI_Hard(Dataset):
    def __init__(self, args, matched=True):
        ...
        self.pattern = r"\b\w+\b"
        if matched:
            self.data = np.load('/data/zhangdacao/dataset/multi_nli/matched_hard.npy', allow_pickle='TRUE')
        else:
            self.data = np.load('/data/zhangdacao/dataset/multi_nli/mismatched_hard.npy', allow_pickle='TRUE')
...
class SNLI_VE_Hard(Dataset):
    def __init__(self, args, matched=True):
        ...
        self.pattern = r"\b\w+\b"
        self.image = np.load('/data/zhangdacao/dataset/snli_ve/image.npy', allow_pickle='TRUE').item()
        self.data = np.load('/data/zhangdacao/dataset/snli_ve/hard.npy', allow_pickle='TRUE')
...
class SNLI_Hard(Dataset):
    def __init__(self, args):
        ...
        f = open('/data/zhangdacao/dataset/snli/test_hard.jsonl', 'r+', encoding='utf-8')
...
def create_snli_ve_hard_dataloaders(args, confactual=0):  
    args.confactual = confactual
    hard_dataset = SNLI_VE_Hard(args)
    test_sampler = SequentialSampler(hard_dataset)
    hard_dataloader = DataLoader(hard_dataset,
                                  batch_size=args.test_batch_size,
                                  sampler=test_sampler,
